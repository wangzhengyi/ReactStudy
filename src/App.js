...
const defaultList = [{
    ...
    rpid: 3, // 用户信息
    user: {
        uid: '13258165', avatar: avatar, uname: '周杰伦',
    }, // 评论内容
    content: '哎哟，不错哦-1', // 评论时间
    ctime: '10-18 08:15', like: 87,
}, {
    rpid: 2, user: {
        uid: '36080105', avatar: avatar, uname: '许嵩',
    }, content: '我寻你千百度 日出到迟暮', ctime: '11-13 11:29', like: 88,
}, {
    rpid: 1, user: {
        uid: '30009257', avatar: avatar, uname: '黑马前端',
...
const App = () => {
    const [commentList, setCommentList] = useState(defaultList.sort((itemA, itemB) => {
    ...
    const getTimeTypeClassName = (type) => {
        if (type === 'time') {
    ...
    const getHotTypeClassName = (type) => {
        if (type === 'hot') {
            return 'nav-item active'
        } else {
            return 'nav-item'
        ...
        <div className="reply-navigation">
            <ul className="nav-bar">
                ...
                <li className="nav-sort">
                    {/* 高亮类名： active */}
                    <span className={getTimeTypeClassName(tabType)} onClick={() => {
                        setTabType("time")
                        setCommentList(commentList.slice().sort((itemA, itemB) => {
                            if (itemB.ctime > itemA.ctime) {
                                return 1;
                            } else if (itemB.ctime < itemA.ctime) {
                                return -1;
                    ...
                    <span className={getHotTypeClassName(tabType)} onClick={() => {
                        setTabType("hot")
                        setCommentList(commentList.slice().sort((itemA, itemB) => {
                            ...
                            <div className="content-wrap">
                                ...
                                <div className="root-reply">
                                    ...
                                    <div className="reply-info">
                                        ...
                                        {comment.user.uid === user.uid && <span className="delete-btn" onClick={() => {
